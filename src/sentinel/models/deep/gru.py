"""GRU predictor anomaly detector.

Trains a GRU network to predict the next timestep from a sliding window
of ``seq_len`` steps. The anomaly score for each window is the MSE between
the predicted and actual next timestep. Higher prediction error indicates
a more anomalous observation.

GRU is a lighter alternative to LSTM — it uses two gates (reset, update)
instead of three (input, forget, output), resulting in fewer parameters
and faster training while retaining comparable performance on many
time-series tasks.
"""
...
if HAS_TORCH:
    ...
    class _GRUNetwork(nn.Module):
        """Internal GRU network for next-step prediction.
    ...
    @register_model("gru")
    class GRUDetector(BaseAnomalyDetector):
        """GRU next-step predictor for anomaly detection.

        Trains a GRU to forecast the next timestep given a window of
        ...
        GRU uses fewer parameters than LSTM (two gates vs. three),
        making it faster to train while delivering similar sequence
        ...
        Example::
            ...
            detector = GRUDetector(hidden_dim=64, seq_len=50)
        ...
        def fit(self, X: np.ndarray, **kwargs: Any) -> None:
            """Train the GRU predictor on data.
            ...
            if n_samples < min_rows:
                raise SentinelError(
                    f"GRU requires at least {min_rows} samples "
        ...
        def score(self, X: np.ndarray) -> np.ndarray:
            ...
            if n_samples < min_rows:
                raise SentinelError(
                    f"GRU scoring requires at least {min_rows} samples "
                    f"(seq_len + 1), got {n_samples}"
                )
        ...
        def _check_fitted(self) -> None:
            ...
            if not self._is_fitted:
                raise SentinelError(
                    "GRUDetector has not been fitted. Call fit() first."
                )
